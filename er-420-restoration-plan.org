...
* TODO Phase -1 — Schematic Prep [6/8]
...
** TODO Walk the whole sheet, fill in ZONES.org's subsystem table
Power supply, Phono EQ, FM front end, FM IF, Ratio detector, AM
section, Multiplex, Tone/preamp, Driver, Output stage, Selector
switch — replace the current placeholder ranges with real ones. Worth
doing for the whole sheet even with aux-only scope right now — the map
is needed later when phono and RF get picked back up.

** TODO Crop sections for what's actually in scope
Aux-only means power supply, the shared tone/preamp/driver chain,
output stage, and the relevant switches — not phono EQ or the
tuner/MPX sections yet. Into =schematic/sections/=, named like
ZONES.org's own example (=G1-H3-power-supply.png=).
...
* TODO Phase 2 — Survey before touching anything [1/14]
